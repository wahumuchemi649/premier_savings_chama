...
class Members(Base):
    ...
    roles = Column(Enum('Admin', 'Member'))
...
class Interest_Distribution(Base):
    ...
    source = Column(Enum('loanInterest', 'bankInterest'))
...
class Loan_Requests(Base):
    ...
    status = Column(Enum('pending_guarantor', 'pending_admin', 'approved', 'rejected'), default='pending_guarantor')
...
class Guarantor_Confirmations(Base):
    ...
    status = Column(Enum('pending', 'confirmed', 'declined'), default='pending')
